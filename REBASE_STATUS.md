...
- **Series-detection crash-proofing (`26e8ad8c`).** Opt-in series scan re-runs on the home render every boot; on a large/odd collection it could OOM mid-scan or hard-crash on a malformed OPF before recording the book → inescapable crash-loop ("Detecting series..." persisting across resets). Fixed with a memory guard (`MemoryBudget::hasHeapForSeriesScan`, bail before popup + per-book) and mark-before-parse (record `("","")` before `extractSeriesFromOpf`, update on success) so one bad book can't repeat forever. A wired flash of this build un-bricks an already-stuck device. (Wired only — OTA can't repartition the new 7.5 MB layout.)
