...
## [flexble-v1.0.0] - 2026-05-17
First stable FlexBLE release post-upstream-merge. FlexBLE now uses its own semver (`flexble_version`) independent of upstream CrossInk's versioning. The `crossink_version` field continues to track the upstream sync point (currently 1.2.11.1).
...
- After a successful BT connection from the in-book Bluetooth menu, FlexBLE now auto-pops both the BT settings and the reader menu so the user lands straight back in the book — no manual back-tapping required.
...
- Auto-retry on chapter-layout abort: if the parser trips the low-heap floor while BLE is consuming its ~58 KB share, FlexBLE silently drops BLE, retries the layout (now with the extra headroom), and lets the existing auto-reconnect logic re-establish the link on the user's next remote button press.

### Changed (FlexBLE fork)
...
- Settings → System now displays `FlexBLE x.y.z (CrossInk x.y.z.z)` so both the fork and upstream sync version are visible.
